...
E^{N}(t_f-1)
\ge
E^{\min}+E_{\mathrm{res}}(t_f,H_f)
}
\tag{F14}
\]

其中 \(E_{\mathrm{res}}\) 表示针对给定 \(G_1\) 降额程度和故障持续时间 \(H_f\) 所预留的 ESS 应急能量。
...
### 6.5 在现有 MPPSO 中的实现方式

对 MPPSO 的每一个正常调度候选粒子，先计算其 \(P_{g,2}^{N}(t)\)、\(E^{N}(t)\) 和 \(V^{N}(t)\)，再利用式 (R1)–(R6) 逐时计算 \(E_{\mathrm{res}}(t,H_r)\)，并检查式 (R7)–(R8)。若违反韧性备用约束，则在适应度函数中加入约束罚项。这样，\(E_{\mathrm{res}}\) 会随候选航速、\(G_2\) 当前出力、负荷和 PV 状态动态变化，无需单独进行外部迭代。
...
2. **固定备用策略（Fixed reserve）**：设置固定 ESS 最低备用；
3. **动态韧性备用策略（Dynamic reserve）**：根据潜在 \(G_1\) 降额程度和设计支撑时长动态确定 \(E_{\mathrm{res}}\)。
...
为了验证动态韧性备用的作用，建议只设置三种调度策略，并在完全相同的故障条件下比较：

1. **无备用策略（No reserve）**：不设置故障备用；
2. **固定备用策略（Fixed reserve）**：设置固定 ESS 最低备用；
3. **动态韧性备用策略（Dynamic reserve）**：采用式 (R1)–(R7) 的动态韧性备用。
...
分别改变实际故障持续时间 \(H_f\) 和降额系数 \(\alpha_F\)，比较三种方法的
